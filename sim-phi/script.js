...
async function checkSupport() {
	...
	if (!await loadPlugin('ogg格式兼容', 'js/oggmented-bundle.js', () => !oggCompatible && isUndefined('oggmented'))) return -4;
...
document.addEventListener('DOMContentLoaded', async function qwq() {
	...
	//修改此处为资源链接地址，将资源转储为本地，防止GitHub连不上的抽象问题
	// const pth = atob('aHR0cHM6Ly9sY2h6aDM0NzMuZ2l0aHViLmlvL2Fzc2V0cy8=');
	const pth = atob('YXNzZXRzLw==');
...
function qwqdraw3(statData) {
	...
	ctxos.fillText(app.speed === 1 ? '' : statData.textAboveStr.replace('{SPEED}', app.speed.toFixed(2)), 383 + Math.min(dxlvl, 750), 208);
